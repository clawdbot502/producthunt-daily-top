...
def fetch_top_products(ph_token: str, limit: int = 10) -> List[Product]:
    ...
    logger.info("[DEBUG] First edge raw node: %s", edges[0]["node"] if edges else "EMPTY")
    products: List[Product] = []
    for rank, edge in enumerate(edges, start=1):
        node = edge["node"]
        topics = [t["node"]["name"] for t in node.get("topics", {}).get("edges", [])]
        logger.info("[DEBUG] #%s %s votesCount=%s commentsCount=%s", rank, node.get("name"), node.get("votesCount"), node.get("commentsCount"))
        products.append(
            Product(
                name=node.get("name", ""),
